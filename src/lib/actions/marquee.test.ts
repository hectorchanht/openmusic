...
import { isOverflowing, marqueeState, MIN_OVERFLOW_PX } from './marquee';
...
describe('marqueeState — measure decision (quick-260712-mkq)', () => {
	...
	it('content fits the box → off, dx 0', () => {
		expect(marqueeState(100, 120, false)).toEqual({ on: false, dx: 0 });
	});
	...
	it('reduced motion is always off, even for a large overflow', () => {
		expect(marqueeState(400, 100, true)).toEqual({ on: false, dx: 0 });
	});
