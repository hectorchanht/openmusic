// Traditional Chinese (繁體中文) UI-chrome dictionary.
// Key set is IDENTICAL to en.ts (enforced by the `Dict` type + the i18n unit test).
import type { Dict } from "./index";

const zhHant: Dict = {
	// --- nav ---
	"nav.home": "首頁",
	"nav.search": "搜尋",
	"nav.library": "音樂庫",

	// --- offline (OFFL-03) ---
	"offline.indicator": "目前離線",
	"update.available": "有新版本可用",
	"update.reload": "重新載入",
	"offline.title": "你目前離線",
	"offline.body": "此功能需要連線。已下載的歌曲仍可播放。",
	"offline.goToLibrary": "前往音樂庫",

	// --- common ---
	"common.back": "返回",
	"common.done": "完成",
	"common.loading": "載入中…",
	"common.retry": "重試",

	// --- home ---
	"home.settings": "設定",
	"home.searchPill": "搜尋歌曲、歌手",
	"home.topPicks": "精選推薦",
	"home.loadingPicks": "載入中…",
	"home.randomize": "隨機推薦",
	"home.noResults": "沒有結果 — 來源可能暫時無法使用",
	"home.topHits": "熱門金曲",
	"home.topArtists": "熱門歌手",
	"home.tagShelf": "{tag}",
	"home.countryShelf": "{country}熱門",
	"home.unplayable": "找不到可播放的版本",
	"home.preparing": "載入中…",

	// --- chart pages (HOME-04) ---
	"charts.topArtists": "熱門歌手",
	"charts.topTracksTab": "歌曲",
	"charts.topArtistsTab": "歌手",

	// --- search ---
	"search.title": "搜尋",
	"search.placeholder": "搜尋歌曲、歌手",
	"search.go": "搜尋",
	"search.submitting": "…",
	"search.someFailed": "部分來源沒有回應 — 顯示其餘結果。",
	"search.searching": "正在搜尋所有來源…",
	"search.loadingMore": "載入更多…",
	"search.empty": "沒有結果。",
	"search.artists": "歌手",
	"search.noMore": "沒有更多結果了。",
	"search.recent": "最近搜尋",
	"search.clear": "清除",
	"search.suggestions": "建議",
	"search.clearInput": "清除搜尋",
	"search.removeRecent": "移除",
	"search.confirmRemoveRecent": "從最近搜尋中移除「{q}」？",
	"search.confirmClearAll": "清除所有最近搜尋？",

	// --- settings ---
	"settings.title": "設定 · openmusic",
	"settings.heading": "設定",
	"settings.appLanguage": "介面語言",
	"settings.lyricsTranslation": "歌詞翻譯",
	"settings.translateArtist": "歌手名稱翻譯",
	"settings.translateTitle": "歌曲名稱翻譯",
	"settings.translateLastfm": "簡介",
	"settings.translateArtistNote": "將顯示的歌手名稱翻譯成此語言。",
	"settings.translateTitleNote": "將顯示的歌曲與專輯名稱翻譯成此語言。",
	"settings.zhScript": "中文字體",
	"settings.zhScriptNote": "無論來源使用哪種字體，都將所有中文歌名、歌手與專輯名統一為同一字體。僅限中文，不影響其他語言。",
	"settings.translateLyricsNote": "將顯示的歌詞翻譯成此語言。",
	"settings.translateTargets": "翻譯對象",
	"settings.translateApplyAll": "全部套用",
	"settings.translateApplyAllNote": "一次把歌詞、歌手名稱、歌曲名稱和簡介都設為同一種語言。展開下方個別項目，即可單獨為它設定不同語言。",
	"settings.translateMixed": "混合",
	"settings.translateLastfmNote": "為歌手簡介選擇語言，或選「自動」以跟隨應用程式語言。",
	"settings.skipLanguages": "跳過這些來源語言",
	"settings.skipLanguagesNote": "已是所勾選語言的文字將保持原文不翻譯。",
	"settings.translateMode": "翻譯模式",
	"settings.translateModeOffNote": "僅影響歌詞。請先在上方選擇歌詞語言以啟用。",
	"settings.translateModeOnNote": "僅適用於歌詞。",
	"settings.lyricsHideParenTranslation": "隱藏括號中的翻譯",
	"settings.lyricsHideParenTranslationNote": "當歌詞中含有（括號）短語（通常是嵌入的翻譯），將其拆到獨立行並隱藏其再翻譯。",
	"settings.lyricsHideParenLines": "隱藏括號中的歌詞",
	"settings.lyricsHideParenLinesNote": "從歌詞中去除括號中的部分,只保留原始行。",
	"settings.defaultQuality": "預設音質",
	"settings.defaultQualityNote": "盡力而為 — 並非所有來源都提供位元率；在已知時會優先選擇。",
	"settings.downloadQuality": "下載音質",
	"settings.downloadQualityNote": "下載歌曲時要求的音質（會以此音質重新解析）。",
	"settings.defaultSource": "預設音樂來源",
	"settings.defaultSourceNote": "當同一首歌出現在多個來源時，優先採用偏好來源。",
	"settings.accentColor": "主題色",
	"settings.theme": "主題",
	"settings.themeDark": "深色",
	"settings.themeLight": "淺色",
	"settings.playbackMotion": "播放與動態",
	"settings.appearanceMotion": "動態效果",
	"settings.autoExpand": "播放時自動展開正在播放",
	"settings.autoExpandDesc": "每次開始播放都會展開完整的播放畫面。",
	"settings.showQualityTag": "在正在播放頁顯示音質標籤",
	"settings.showQualityTagDesc": "來源未提供音質時不會顯示。",
	"settings.nowbarLyrics": "在迷你播放器中顯示歌詞",
	"settings.reduceMotion": "減少動態效果",
	"settings.shareIncludeTitle": "分享時附上標題",
	"settings.data": "資料",
	"settings.dataCounts": "{liked} 個收藏 · {playlists} 個播放清單 · {downloads} 個下載",
	"settings.clearPicks": "清除快取的精選推薦",
	"settings.clearLibrary": "清空音樂庫",
	"settings.migrateDownloads": "將下載移至 Download/openmusic",
	"settings.migrateDownloadsDesc": "將已下載的檔案移至新的 Download/openmusic 資料夾並更新索引。只需執行一次；可安全地再次點按。",
	"settings.migrateDownloadsResult": "已移動 {moved} / {total}",
	"settings.about": "關於",
	"settings.aboutLine": "openmusic · 示範版本 · 串流自公開音樂來源",
	"settings.optOff": "關閉",
	"settings.optOn": "開啟",
	"settings.optShowBelow": "顯示於下方",
	"settings.optReplace": "取代",
	"settings.optAuto": "自動",
	"settings.optLossless": "無損",
	"settings.optAutoAll": "自動（全部）",
	"settings.picksCleared": "已清除快取的精選推薦。",
	"settings.libraryCleared": "已清空音樂庫。",
	"settings.clearLibraryConfirm": "確定清除所有收藏歌曲、播放清單與下載嗎？",

	// --- settings groups / history / lastfm ---
	"settings.groupGeneral": "一般",
	"settings.groupTranslation": "翻譯",
	"settings.groupPlayback": "播放",
	"settings.groupHistory": "播放紀錄",
	"settings.groupLastfm": "Last.fm",
	"settings.groupData": "資料",
	"settings.groupAbout": "關於",
	"settings.groupGeneralDesc": "語言與分享",
	"settings.groupTranslationDesc": "歌詞與名稱翻譯",
	"settings.groupPlaybackDesc": "音質、來源、自動展開",
	"settings.groupHistoryDesc": "最近播放",
	"settings.groupLastfmDesc": "同步記錄 — 即將推出",
	"settings.groupDataDesc": "音樂庫統計、清除資料",
	"settings.groupAboutDesc": "版本資訊",
	"settings.backToSettings": "設定",

	// --- activity log (quick-260630-sgw) ---
	"settings.groupActivity": "活動記錄",
	"settings.groupActivityDesc": "詳細的播放器操作記錄",
	"settings.activityHeading": "活動記錄",
	"settings.activityEmpty": "尚無記錄的活動。",
	"settings.activityClear": "清除記錄",
	"settings.activityCopy": "複製記錄",
	"settings.activityCopied": "已複製活動記錄。",
	"settings.activityUpload": "上傳記錄",
	"settings.activityUploadPrompt": "貼上診斷上傳權杖",
	"settings.activityUploaded": "已上傳記錄。",
	"settings.activityUploadFailed": "上傳失敗。",
	"settings.activityUploadEmpty": "沒有可上傳的內容。",

	// --- downloads settings / retag (36-D-17) ---
	"settings.groupDownloads": "下載",
	"settings.groupDownloadsDesc": "為下載的檔案寫入歌曲資訊",
	"settings.retagDownloads": "重寫下載歌曲的資訊（{count}）",
	"settings.retagDownloadsDesc": "為本應用程式仍保存的離線副本重新寫入歌名、歌手、專輯和封面。在 Android 上也會重寫 Music/OpenMusic 中的檔案。透過瀏覽器下載資料夾儲存的檔案無法在此處理。",
	"settings.retagConfirm": "為 {count} 個下載的檔案寫入歌曲資訊？無法寫入的檔案會保持原樣。",
	"settings.retagImported": "其中 {count} 首是從這台手機匯入的歌曲。它們的檔案會在原處重寫——同一個檔案、同一個名稱，不會產生副本。",
	"settings.retagProgress": "正在寫入第 {done} 個，共 {total} 個…",
	"settings.retagDone": "已寫入 {tagged} 個，共 {total} 個。略過 {skipped} 個。",
	"settings.retagNone": "沒有可重寫資訊的離線下載。",
	"settings.excludedHeading": "你標記為不匯入的檔案",
	"settings.excludedNone": "尚未標記任何檔案。",
	"settings.excludedNote": "這些檔案仍在你的手機上 — 只是被排除在匯入之外。重新允許後，下次掃描時會回來。",
	"settings.excludedRestore": "重新允許",

	// --- appearance / sizing (quick-260607-fnp) ---
	"settings.groupAppearance": "外觀",
	"settings.groupAppearanceDesc": "主題、強調色、文字與封面大小",
	"settings.appearanceText": "文字大小",
	"settings.appearanceLayout": "封面與版面",
	"settings.rowButtons": "歌曲列按鈕",
	"settings.rowButtonsDesc": "選擇清單中歌曲旁要顯示哪些按鈕，以及排列順序。拖曳可調整位置，點按可關閉。⋮ 選單永遠都在，因此即使全部關閉，所有操作仍可觸及。",
	"settings.rowButtonOn": "{name} — 已顯示，第 {pos} 個，共 {total} 個。按 Enter 隱藏，或用左右方向鍵移動。",
	"settings.rowButtonOff": "{name} — 已隱藏。按 Enter 顯示。",
	"settings.fontSizeApp": "應用程式文字大小",
	"settings.fontSizeAppDesc": "一次縮放應用程式中的所有文字。下方「歌曲列表」與「正在播放」的大小會在此基礎上再相乘。",
	"settings.songRowEditor": "歌曲列表",
	"settings.songRowEditorDesc": "點按範例列中反白的部分即可選取，然後拖曳滑桿 — 該列會隨拖曳即時改變大小。拖曳按鈕可移動位置，點按可關閉；⋮ 選單永遠都在。",
	"settings.npEditor": "正在播放",
	"settings.npEditorDesc": "這些大小只影響全螢幕的「正在播放」畫面。點按反白的部分，然後拖曳滑桿 — 變更會立即生效。",
	"settings.wingPick": "{name}，{value}%。點按以調整這個部分的大小。",
	"settings.wingPicked": "{name}，{value}%，已選取。使用下方的滑桿調整大小。",
	"settings.editorSlider": "{name} 的大小",
	"settings.fontSizeTitle": "歌曲標題",
	"settings.fontSizeArtist": "歌手名稱",
	"settings.fontSizeLyrics": "歌詞",
	"settings.fontSizeNpTitle": "正在播放 — 歌曲名",
	"settings.fontSizeNpArtist": "正在播放 — 歌手",
	"settings.coverSize": "封面大小",
	"settings.gridColumns": "首頁格狀欄數",
	"settings.appearanceNote": "大小會套用至整個應用程式。100% 為預設值。",
	"settings.preview": "預覽",
	"settings.aboutSetting": "關於此設定",
	"settings.clearNameCache": "清除名稱翻譯快取",
	"settings.clearCoverCache": "清除封面快取",
	"settings.clearCoverCacheHint": "刪除已儲存的封面，下次檢視時會重新擷取缺失或過時的封面。",
	"settings.clearSearchHistory": "清除搜尋紀錄",
	"settings.resetAppearance": "重設外觀",
	"settings.resetGroup": "還原預設值",
	"settings.resetConfirm": "將此分頁還原為預設值？",
	"settings.resetDone": "已還原預設值。",
	"settings.nameCacheCleared": "已清除名稱翻譯快取。",
	"settings.coverCacheCleared": "已清除封面快取。",
	"settings.searchHistoryCleared": "已清除搜尋紀錄。",
	"settings.appearanceReset": "已重設外觀大小。",
	"settings.lyricsTranslateMode": "歌詞翻譯模式",
	"settings.bioAuto": "自動（依應用程式語言）",

	// --- settings: home layout (quick-260606-w87) ---
	"settings.groupHome": "首頁版面",
	"settings.groupHomeDesc": "排行榜、地區、區塊、版面",
	"settings.homeSections": "區塊與排序",
	"settings.homeSectionTopHits": "熱門金曲",
	"settings.homeSectionTopArtists": "熱門歌手",
	"settings.homeSectionTags": "曲風專區",
	"settings.homeSectionCountries": "地區專區",
	"settings.homeSectionLiked": "我喜歡的歌",
	"settings.homeSectionDownloads": "已下載",
	"settings.homeSectionFavArtists": "收藏的歌手",
	"settings.homeSectionPlaylists": "播放清單",
	"settings.homeSectionHistory": "最近播放",
	"settings.homeSectionRadio": "你的電台",
	"settings.dragToReorder": "拖曳握把以重新排序 · 切換以顯示／隱藏",
	"settings.homeGenres": "曲風",
	"settings.homeShowingAll": "未選擇任何項目 — 顯示全部",
	"settings.homeDragReorderChips": "點按切換 · 拖曳已選項目可重新排序",
	// --- 39: chart shelves ---
	"settings.homeSectionChartSongs": "熱門歌曲",
	"settings.homeSectionNewReleases": "最新發行",
	"settings.homeSectionChartArtists": "熱門歌手",
	"settings.homeSectionChartAlbums": "熱門專輯",
	"settings.homeSectionYtTrending": "YouTube 熱門",
	"settings.homeSectionGenres": "曲風排行榜",
	"settings.homeSrcLibrary": "你的音樂庫",
	"settings.homeSrcUnavailable": "{region}暫不提供",
	"settings.homeGroupCharts": "排行榜",
	"settings.chartRegion": "排行榜地區",
	"settings.chartRegionDesc": "選擇由哪個國家或地區的排行榜提供熱門歌曲、最新發行、熱門歌手、熱門專輯及 YouTube 熱門。「自動」會跟隨介面語言。",
	"settings.chartRegionAuto": "自動（{region}）",
	"settings.moreRegions": "更多地區",
	"settings.moreRegionsDesc": "為你選擇的每個額外地區各加入一個熱門歌曲專區，按顯示的順序排列。",
	"settings.moreRegionsNone": "無",
	"settings.chartGenresDesc": "你選擇的每個曲風都會在首頁成為獨立的排行榜專區。",
	"settings.chartGenresNone": "未選擇任何曲風 — 首頁不會顯示曲風專區",
	"settings.homeGroupClassic": "經典（Last.fm / Deezer）",
	"settings.homeClassicDesc": "舊版本的首頁專區，資料來自 Deezer 及 Last.fm。預設為關閉；開啟任何一項即可原樣恢復。",
	"settings.homeClassicOnCount": "已開啟 {n} 項",
	"settings.itemsPerShelf": "每排項目數 · {n}",
	"settings.defaultLandingTab": "預設起始分頁",
	"settings.landingHome": "首頁",
	"settings.landingSearch": "搜尋",
	"settings.landingLibrary": "音樂庫",
	"settings.tileDensity": "圖磚密度",
	"settings.densityComfortable": "舒適",
	"settings.densityCompact": "緊湊",
	"settings.densityList": "清單",
	"settings.densityPile": "封面",
	"settings.densityGrid": "網格",
	"settings.homeChrome": "首頁控制項",
	"settings.showSearchPill": "顯示搜尋列",
	"settings.showRandomize": "顯示隨機推薦按鈕",

	"history.title": "播放紀錄 · openmusic",
	"history.heading": "播放紀錄",
	"history.empty": "尚無最近播放的歌曲。",
	"history.clear": "清除播放紀錄",
	"history.cleared": "已清除播放紀錄。",
	"lastfm.title": "Last.fm · openmusic",
	"lastfm.heading": "Last.fm",
	"lastfm.connect": "連接 Last.fm",
	"lastfm.comingSoon": "即將推出",
	"lastfm.note": "同步記錄與你的 Last.fm 個人檔案將於未來版本推出。",
	"lastfm.readMore": "在 Last.fm 上閱讀更多",
	"lastfm.about": "簡介",
	"lastfm.listeners": "聽眾",
	"lastfm.playcount": "播放次數",

	// --- now playing ---
	"nowplaying.collapse": "收合",
	"nowplaying.options": "選項",
	"nowplaying.nowPlaying": "正在播放",
	"nowplaying.albumArt": "專輯封面 — 向下拖曳以縮小",
	"nowplaying.seek": "調整進度",
	"nowplaying.shuffle": "隨機播放",
	"nowplaying.previous": "上一首",
	"nowplaying.playPause": "播放／暫停",
	"nowplaying.next": "下一首",
	"nowplaying.repeat": "重複播放",
	"nowplaying.repeatModeOne": "單曲循環",
	"nowplaying.collapsePanel": "收合面板",
	"nowplaying.expandPanel": "展開面板",
	"nowplaying.upNext": "待播清單",
	"nowplaying.lyrics": "歌詞",
	"nowplaying.related": "相關",
	"nowplaying.reorderTrack": "重新排序歌曲",
	"nowplaying.noQueue": "尚無待播歌曲。",
	"nowplaying.skippedRetry": "已略過 — 點擊重試",
	"nowplaying.translating": "翻譯中…",
	"nowplaying.noLyrics": "這首歌沒有歌詞。",
	"nowplaying.loadingRelated": "載入相關歌曲中…",
	"nowplaying.noRelated": "沒有相關歌曲。",
	"nowplaying.lastfmTags": "Last.fm 標籤",

	// --- now bar ---
	"nowbar.openNowPlaying": "開啟正在播放",
	"nowbar.playPause": "播放／暫停",
	"nowbar.volume": "音量",
	"nowbar.mute": "靜音",
	"nowbar.unmute": "取消靜音",

	// --- track menu ---
	"menu.playNext": "下一首播放",
	"menu.addToQueue": "加入待播清單",
	"menu.download": "下載",
	"menu.downloaded": "已下載",
	"menu.unavailable": "檔案遺失",
	"menu.like": "收藏",
	"menu.liked": "已收藏",
	"menu.addToPlaylist": "加入播放清單",
	"menu.goToAlbum": "前往專輯",
	"menu.goToArtist": "前往歌手",
	"menu.share": "分享",
	"menu.detail": "詳細資料",
	"menu.newPlaylist": "新增播放清單…",
	"menu.newPlaylistPrompt": "新播放清單名稱",
	"menu.closeMenu": "關閉選單",
	"menu.close": "關閉",
	"menu.trackDetail": "歌曲詳細資料",
	"menu.detailTitle": "標題",
	"menu.detailArtist": "歌手",
	"menu.detailAlbum": "專輯",
	"menu.detailQuality": "音質",
	"menu.detailFile": "檔案",
	"menu.detailSource": "來源",
	"menu.detailUid": "UID",
	"menu.detailAudioUrl": "音訊網址",
	"menu.detailNotResolved": "（尚未解析）",
	"menu.detailUnknown": "未知",
	"menu.sleepTimer": "睡眠定時器",
	"menu.remix": "重新混音",
	"menu.preparing": "準備中…",

	// --- version picker (Phase 26-04, VERSIONS-01) ---
	"versions.title": "從音源播放",
	"versions.empty": "沒有其他版本",
	"versions.unknownQuality": "未知音質",
	"versions.open": "選擇音源",
	"versions.loading": "尋找版本…",
	"menu.versions": "從音源播放",
	"menu.changeCover": "更換封面",
	"menu.fixLyrics": "修正歌詞",
	"menu.lyricsPickerNone": "找不到這首歌的歌詞",
	"menu.lyricsRetry": "重試",
	"menu.lyricsAuto": "使用自動歌詞",
	"menu.editTags": "編輯歌曲資訊",
	"menu.noImport": "不再匯入",
	"menu.removeDownload": "移除下載",
	"menu.removeDownloadBody": "歌曲仍留在你的音樂庫中，輕觸後會重新線上播放。透過瀏覽器下載資料夾儲存的檔案，無法從這裡處理。",
	"menu.removeDownloadDeleteFile": "同時刪除此裝置上的離線副本",
	"menu.removeDownloadAction": "移除",
	"tags.title": "編輯歌曲資訊",
	"tags.fieldTitle": "歌曲名稱",
	"tags.fieldArtist": "歌手",
	"tags.fieldAlbum": "專輯",
	"tags.fieldFilename": "檔案名稱",
	"tags.filenameHint": "只影響手機上的檔案。留空則依歌名和歌手命名。無論哪種方式，檔案格式都不變。",
	"tags.hint": "資訊會寫入檔案本身，連同本應用程式目前顯示的封面與歌詞。留空表示保留檔案中原有的內容。",
	"tags.save": "儲存",
	"tags.cancel": "取消",
	"tags.saving": "正在儲存…",
	"menu.coverPickerNone": "找不到封面",
	"menu.downloadFrom": "從來源下載…",
	"menu.downloadHoldHint": "長按選擇來源與格式",
	"menu.downloadUnavailable": "無法取得",
	"versions.tag.live": "現場",
	"versions.tag.acoustic": "原音",
	"versions.tag.demo": "試唱",
	"versions.tag.cover": "翻唱",
	"versions.tag.remix": "混音",
	"versions.tag.instrumental": "純音樂",
	"versions.tag.remaster": "重製版",

	// --- sleep timer (Phase 18, TIMER-01) ---
	"timer.minutes": "{n} 分鐘",
	"timer.endOfTrack": "本曲結束",
	"timer.cancel": "取消定時器",

	// --- toasts ---
	"toast.playingNext": "下一首播放",
	"toast.addedToQueue": "已加入待播清單",
	"toast.sharedPlaying": "正在播放分享的歌曲 · 待播清單保留不變",
	"toast.removedFromQueue": "已從待播清單移除",
	"toast.undo": "復原",
	"toast.preparingDownload": "正在準備下載…",
	"toast.noAudio": "沒有可用的音訊",
	"toast.skipped": "無法播放 · {title} — 已略過",
	"toast.skippedMany": "已略過 {count} 首歌曲",
	"toast.playbackStopped": "播放已停止 — 無法載入歌曲",
	"toast.retry": "重試",
	"toast.offlineNoDownloads": "你目前離線 — 沒有已下載的歌曲可播放",
	"toast.downloaded": "已下載 · 已加入音樂庫",
	"toast.openedAudio": "已開啟音訊 · 已加入音樂庫",
	"toast.downloadFailedKeptInLibrary": "無法儲存 · 已保留在音樂庫",
	"toast.downloadCorrupted": "下載檔案已損毀 · 改為線上播放",
	"toast.shareCopied": "已複製分享連結",
	"toast.liked": "已收藏",
	"toast.coverPinned": "封面已更新",
	"toast.lyricsPinned": "歌詞已更新",
	"toast.tagsSaved": "歌曲資訊已寫入檔案",
	"toast.tagsFailed": "無法儲存歌曲資訊 — 檔案未變更",
	"toast.noImportDone": "已從下載清單移除。檔案仍在你的手機上，不會再被匯入。",
	"toast.downloadRemoved": "已移除下載。歌曲仍留在你的音樂庫中。",
	"toast.noImportUndone": "已重新允許 — 下次匯入時會回來",
	"toast.unliked": "已取消收藏",
	"toast.remixing": "正在以這首歌重新混音",
	"toast.artistFavorited": "已加入收藏的歌手",
	"toast.artistUnfavorited": "已從收藏的歌手中移除",
	"settings.sourcesAdvanced": "進階 — 音樂來源",
	"settings.sourcesAdvancedNote": "停用某來源可將其隱藏於搜尋結果。",
	"menu.shuffleQueue": "隨機播放佇列",
	"menu.clearQueue": "清除佇列",
	"toast.addedToPlaylist": "已加入播放清單",
	"toast.playlistCreated": "已建立播放清單",
	"toast.importDone": "匯入完成 · 已加入 {count} 首歌曲",
	"toast.importFailed": "無法讀取裝置音樂庫",
	"toast.patternFellBack": "自訂樣式太慢 · 已改用預設值",
	"toast.fileMissing": "檔案遺失 · 重新匯入即可還原",

	// --- library ---
	"library.title": "音樂庫 · openmusic",
	"library.heading": "音樂庫",
	"library.liked": "收藏",
	"library.playlists": "播放清單",
	"library.downloads": "下載",
	"library.favArtists": "收藏的歌手",
	"library.noLiked": "尚無收藏歌曲。在播放器點 ♥ 收藏。",
	"library.noPlaylists": "尚無播放清單。從歌曲的 ⋮ 選單建立一個。",
	"library.noDownloads": "尚無下載。在歌曲的 ⋮ 選單使用「下載」。",
	"library.noFavArtists": "尚未收藏歌手。在歌手頁面點擊愛心。",
	"library.emptyPlaylist": "空的播放清單。",
	"library.deletePlaylist": "刪除播放清單",
	"library.playAll": "全部播放",
	"library.edit": "編輯",
	"library.remove": "移除",
	"library.clearList": "全部清除",
	"library.clearListConfirm": "確定要清空「{name}」的全部內容嗎？此操作無法復原。",
	"library.deletePlaylistConfirm": "刪除播放清單「{name}」？",
	"library.downloadsNote": "已下載的檔案儲存在你的裝置上。此清單會引用並在點擊時重新串流（網頁應用無法離線播放任意已儲存的檔案）。",

	// --- artist ---
	"artist.back": "‹ 返回",
	"artist.derived": "由跨來源搜尋衍生 · {count} 首歌曲",
	"artist.loading": "載入 {name} 中…",
	"artist.albums": "專輯",
	"artist.trackOne": "{count} 首歌曲",
	"artist.trackMany": "{count} 首歌曲",
	"artist.albumLabel": "專輯",
	"artist.seeAllAlbums": "查看全部",
	"artist.typeAlbum": "專輯",
	"artist.typeEp": "迷你專輯",
	"artist.typeSingle": "單曲",
	"artist.typeCompilation": "合輯",
	"artist.discography": "全部作品",
	"artist.filterAll": "全部",
	"artist.filterMain": "專輯與迷你專輯",
	"artist.filterSingle": "單曲",
	"artist.discographyEmpty": "找不到這位歌手的作品。",
	"artist.hitSongs": "熱門歌曲",
	"artist.showMore": "顯示更多",
	"artist.moreLikeThis": "相似歌手",
	"artist.noSongs": "找不到 {name} 的歌曲。",
	"artist.favorite": "收藏",
	"artist.unfavorite": "已收藏",
	"artist.playArtist": "播放",
	"artist.share": "分享",

	// --- album ---
	"album.title": "{name} · openmusic",
	"album.back": "返回",
	"album.derived": "專輯 · {count} 首歌曲（由搜尋衍生）",
	"album.tracklistNote": "專輯 · {count} 首歌曲",
	"album.loading": "載入專輯中…",
	"album.noTracks": "找不到「{name}」的歌曲。",
	"album.openFromArtist": "請從歌手頁面開啟此專輯以查看曲目清單。",
	"album.unplayable": "找不到可播放的版本",

	// --- Phase 17: up-next sourcing + deezer enrichment + queue clear ---
	"settings.upnextSourcing": "接下來播放的來源",
	"settings.upnextSameList": "同一清單",
	"settings.upnextGenerated": "依曲風生成",
	"settings.ctxSearch": "搜尋",
	"settings.ctxAlbum": "專輯",
	"settings.ctxArtist": "歌手",
	"settings.ctxHomeDiscovery": "首頁",
	"settings.demoPrefix": "範例 {name}",
	"nowplaying.clearQueue": "清除",
	"deezer.fans": "粉絲",
	"deezer.albums": "專輯",
	"deezer.released": "發行日期",
	"deezer.label": "廠牌",
	"deezer.genres": "曲風",
	"deezer.tracks": "曲目",
	"deezer.duration": "時長",
	"home.seeAll": "查看全部",
	// --- 39: chart shelves ---
	"home.chartSongs": "{region}熱門歌曲",
	"home.newReleases": "{region}最新發行",
	"home.chartArtists": "{region}熱門歌手",
	"home.chartAlbums": "{region}熱門專輯",
	"home.ytTrending": "{region} YouTube 熱門",
	"home.genre.cantopop": "廣東歌",
	"home.genre.mandopop": "華語流行",
	"home.genre.kpop": "韓國流行",
	"home.genre.jpop": "日本流行",
	"home.genre.hiphop": "嘻哈",
	"home.genre.rock": "搖滾",
	"home.genre.dance": "舞曲",
	"home.genre.rnb": "R&B",
	"home.genre.electronic": "電子",
	"home.genre.alternative": "另類",
	"home.genre.asian": "亞洲音樂",
	"menu.options": "選項",
	"charts.topTitle": "熱門金曲",
	"charts.tagTitle": "{tag}",
	"charts.countryTitle": "{country}",
	"settings.homeSectionDensity": "版面",
	// general settings helper text (quick-260630-fce)
	"settings.appLanguageDesc": "設定應用程式按鈕、選單與標籤的顯示語言；歌曲與歌手名稱則依照「翻譯」設定，而非此項。",
	"settings.accentColorDesc": "用於作用中的控制項、開關以及正在播放歌曲的強調色；挑一個你喜歡的顏色。",
	"settings.reduceMotionDesc": "關閉應用程式中非必要的動畫與轉場效果；若動態效果讓你不適或想省電，請開啟此項。",
	"settings.shareIncludeTitleDesc": "分享時在連結上方加上一行標題（歌曲 • 歌手）。關閉時只傳送連結——連結預覽本身已會顯示標題。",
	// appearance settings helper text (quick-260630-fce)
	"settings.fontSizeTitleDesc": "縮放清單與搜尋結果中顯示的歌曲標題文字，100% 為預設值；放大以利閱讀，縮小則可在畫面上顯示更多內容。",
	"settings.fontSizeArtistDesc": "縮放清單與搜尋結果中顯示的歌手名稱文字，100% 為預設值；放大以利閱讀，縮小則可在畫面上顯示更多內容。",
	"settings.fontSizeLyricsDesc": "縮放正在播放畫面上顯示的歌詞行，100% 為預設值；放大以便閱讀，縮小則可一次顯示更多行。",
	"settings.fontSizeNpTitleDesc": "縮放全螢幕正在播放畫面上顯示的歌曲標題，100% 為預設值；可依喜好調整，不會影響清單中的文字。",
	"settings.fontSizeNpArtistDesc": "縮放全螢幕正在播放畫面上顯示的歌手名稱，100% 為預設值；可依喜好調整，不會影響清單中的文字。",
	"settings.coverScaleDesc": "縮放各網格與清單中的專輯封面圖；封面越大越容易看清，但每列能放的越少，封面越小則能放越多。",
	"settings.gridColumnsDesc": "設定首頁網格使用的欄數；欄數越多，圖塊越小、每螢幕顯示越多，欄數越少則圖塊越大。",
	// home settings helper text (quick-260630-fce)
	"settings.itemsPerShelfDesc": "每個首頁區塊在你開啟查看更多之前所顯示的項目數量；調高可一眼瀏覽更多，調低則讓頁面更精簡。",
	"settings.defaultLandingTabDesc": "開啟應用程式時預設進入的分頁——首頁、搜尋或音樂庫；設定為你通常最先使用的那一個。",
	"settings.tileDensityDesc": "書架的版面配置，與「每列項目數」不同，後者只是數量。",
	"settings.showRandomizeDesc": "播放全新的隨機組合。",
	// playback settings helper text (quick-260630-fce)
	// data settings helper text (quick-260630-fce)
	"settings.clearPicksDesc": "清除快取的精選推薦與首頁音樂庫區塊，下次將重新產生；若覺得推薦內容過時可使用此項。",
	"settings.clearNameCacheDesc": "清除已儲存的歌手與標題名稱翻譯，使其重新取得；變更翻譯設定後、或某個翻譯名稱有誤時可使用。",
	"settings.clearSearchHistoryDesc": "從此裝置刪除你儲存的過往搜尋記錄，使最近搜尋清單重新清空；此操作不會影響你的音樂庫。",
	"settings.resetAppearanceDesc": "將主題、強調色、減少動態效果、所有文字大小與封面大小還原為預設值；不會影響你的音樂庫或其他設定。",
	"settings.clearLibraryDesc": "永久刪除此裝置上你所有的收藏歌曲、播放清單與下載；此操作無法復原，若想保留請先匯出備份。",
	// --- backup (Settings → Data, phase 35) ---
	"backup.export": "匯出備份",
	"backup.exportDesc": "將你的收藏歌曲、播放清單、下載清單、播放紀錄、搜尋紀錄、設定與名稱翻譯儲存到單一 JSON 檔案。已下載的音訊不會包含在內。",
	"backup.exported": "備份已匯出。",
	"backup.exportFailed": "匯出失敗。",
	"backup.import": "匯入備份",
	"backup.importDesc": "以備份檔的內容取代上述全部資料，然後重新載入應用程式。之後你可以復原上一次匯入。",
	"backup.importConfirm": "要以此檔案的內容取代此裝置上你的收藏歌曲、播放清單、下載清單、播放紀錄、搜尋紀錄、設定與名稱翻譯嗎？",
	"backup.imported": "備份已還原——正在重新載入…",
	"backup.importedSkipped": "備份已還原（已略過 {n} 筆不支援的項目）——正在重新載入…",
	"backup.errNotOurs": "此檔案不是 OpenMusic 的備份。",
	"backup.errNewer": "此備份由較新版本的應用程式建立。請先更新應用程式再試一次。",
	"backup.errDamaged": "此備份檔已損毀，無法匯入。",
	"backup.errNoSnapshot": "無法儲存復原快照，因此未做任何變更。請先釋出儲存空間再試一次。",
	"backup.errWriteFailed": "匯入在寫入時失敗；你先前的資料已還原。",
	"backup.undo": "復原上一次匯入",
	"backup.undoDesc": "將上一次匯入前此裝置上的資料放回，然後重新載入。",
	"backup.undoConfirm": "要復原上一次匯入並還原先前的資料嗎？",
	"backup.undone": "匯入已復原——正在重新載入…",
	"backup.redownload": "重新下載缺少的歌曲（{n}）",
	"backup.redownloadDesc": "逐首下載你的下載清單中音訊不在此裝置上的每首歌曲。可放心重複執行；點按「停止」即可暫停。",
	"backup.stop": "停止重新下載",
	"backup.sweepDone": "已重新下載 {saved} 首，{failed} 首失敗。",

	// --- device import (Phase 34: Settings → Downloads) ---
	"import.heading": "裝置匯入",
	"import.cta": "從裝置匯入歌曲",
	"import.ctaNote": "掃描你的 Music 和 Download 資料夾。檔案會留在原處 — 不會被複製或移動。",
	"import.webOnly": "僅在 Android 應用程式中可用。",
	"import.permDenied": "權限遭拒 — OpenMusic 無法讀取你的音樂檔案。點一下匯入再試一次。",
	"import.permBlocked": "Android 已封鎖此請求。開啟 設定 → 應用程式 → OpenMusic → 權限 → 音樂和音訊，允許後再點一下匯入。",
	"import.scanning": "掃描中…",
	"import.scanProgress": "已檢查 {done} / {total} 個檔案",
	"import.cancel": "取消",
	"import.summaryAdded": "已加入 {count} 首歌曲",
	"import.summaryRelinked": "{count} 首歌曲已重新連結到本裝置上既有的檔案",
	"import.summaryRemoved": "已移除 {count} 首 — 這些檔案已不在本裝置上",
	"import.summaryCancelled": "匯入已取消 — 停止前已加入 {count} 首歌曲。",
	"import.skipTooShort": "已略過 {count} 首 — 短於 {seconds} 秒",
	"import.skipExtension": "已略過 {count} 首 — 該檔案類型已關閉",
	"import.skipRule": "已略過 {count} 首 — 符合略過規則",
	"import.skipExcluded": "已略過 {count} 首 — 你標記為不匯入",
	"import.skipAlready": "{count} 首已在你的音樂庫中",
	"import.none": "找不到歌曲",
	"import.noneBody": "Music 或 Download 中沒有符合規則的檔案。請檢查下方的檔案類型和最短長度，然後再試一次。",
	"import.rules": "匯入規則",
	"import.rulesNote": "預設值已經夠用 — 只有檔名比較特殊時才需要調整。",
	"import.parsing": "檔名解析",
	"import.parsingNote": "僅在檔案本身沒有內嵌標籤時使用。",
	"import.stripTrackNo": "移除開頭的曲目編號",
	"import.stripBrackets": "移除括號內的標記",
	"import.customPattern": "自訂樣式（進階）",
	"import.customPatternNote": "含具名群組的正規表示式 — (?<artist>…) 和 (?<title>…)。符合檔名時優先使用，其餘交給預設處理。",
	"import.patternPreview": "預覽",
	"import.patternInvalid": "不是有效的樣式 — 仍在使用你上一個可用的樣式。",
	"import.patternNoGroups": "至少需要一個具名群組：(?<title>…)、(?<artist>…)、(?<album>…) 或 (?<track>…)。",
	"import.patternTooSlow": "這個樣式在整個音樂庫上執行太慢。請簡化後再試一次。",
	"import.minLength": "最短長度",
	"import.minLengthNote": "更短的檔案會被視為鈴聲或通知音效並略過。",
	"import.fileTypes": "檔案類型",
	"import.fileTypesNote": "只匯入這些副檔名的檔案。",
	"import.skipRules": "略過規則",
	"import.skipRulesNote": "每行一個字詞或片語。檔名或資料夾名稱包含其中任一個時就會略過。",
};

export default zhHant;
