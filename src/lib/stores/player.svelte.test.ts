...
beforeEach(() => {
	...
	player.current = null;
	// quick-260618-t7p: displayed is the deferred now-playing surface track — reset so a prior
	// test's displayed song can't leak into the cold-start-seed / commit-point assertions.
	player.displayed = null;
...
describe('player.displayed — deferred now-playing/nowbar UI swap (quick-260618-t7p)', () => {
	// `player.displayed = null` narrows the field to the `null` literal in TS flow analysis; reads
	// after an opaque play()/restore() then trip "Property 'uid' does not exist on type 'never'".
	// Read through this accessor (the field's true union type) so the assertions type-check.
	const displayed = () => (player as unknown as { displayed: Track | null }).displayed;
	// `displayed` is the track shown on the now-playing SURFACES (Nowbar + NowPlaying meta/cover/
	// lyrics). It advances to `current` ONLY on a real `playing` event (the takeover commit) — a new
	// click leaves the OLD song displayed while the new one loads / if it fails. A COLD-START seed in
	// play() entry (`displayed ??= track`, null-only) shows the very first song before `playing`, and
	// restore() sets it directly (a reload resume, not a load-over-old swap).
	beforeEach(() => {
		player.displayed = null;
		player.current = null;
		player.queue = [];
	...
	it('Test A — commit point: firing `playing` advances displayed to current (a new click mid-playback)', () => {
		const a = mk('netease', 'da', 'A', 'Old Song');
		const b = mk('qq', 'db', 'B', 'New Song');
		// Simulate: song A is displayed; the user clicked B mid-playback so current is now B but the
		// old A is still on the surfaces (no `playing` yet).
		player.displayed = a;
		player.current = b;

		const el = makeFakeAudio();
		player.attach(el as unknown as HTMLAudioElement);
		expect(displayed()?.uid).toBe(a.uid); // still A — the new song hasn't taken over yet

		el.fire('playing'); // B's audio is actually producing output → the takeover commit

		expect(displayed()?.uid).toBe(b.uid); // displayed advanced to current (B)
	});

	it('Test B — keep-old on no playing: a play(B) that never fires `playing` leaves displayed on A', async () => {
		// Restore the REAL play() so the cold-start `??=` seed runs (the global beforeEach stub skips it).
		(player.play as unknown as { mockRestore(): void }).mockRestore?.();
		mockEnsure.mockReset();
		const a = mk('netease', 'ea', 'A', 'Old Song');
		const stubB = stub('qq', 'eb', 'B', 'New Song');
		const resolvedB: Track = { ...mk('qq', 'eb', 'B', 'New Song'), audioUrl: 'https://cdn/b.mp3' };

		const el = makeFakeAudio();
		player.attach(el as unknown as HTMLAudioElement);
		vi.spyOn(library, 'isDownloaded').mockReturnValue(false);

		// A is already displayed + current (it is playing).
		player.displayed = a;
		player.current = a;

		mockEnsure.mockResolvedValue(resolvedB);
		await player.play(stubB); // current becomes B, but NO `playing` event is ever fired
		await flush();

		expect(player.current?.uid).toBe(resolvedB.uid); // internal current advanced (queue/MediaSession)
		expect(displayed()?.uid).toBe(a.uid); // the OLD song stays displayed while B loads / if it fails
	});

	it('Test C — cold start: a first play with displayed null seeds displayed = current synchronously', async () => {
		// Restore the REAL play() so the cold-start `??=` seed runs.
		(player.play as unknown as { mockRestore(): void }).mockRestore?.();
		mockEnsure.mockReset();
		const c = stub('netease', 'fc', 'C', 'First Ever');
		// ensureTrackDetails never settles — prove the seed is synchronous (set on entry, not on land).
		mockEnsure.mockReturnValue(new Promise(() => {}));
		...
		player.displayed = null; // no song was ever displayed (cold start)
		void player.play(c); // do NOT await — assert the synchronous cold-start seed

		expect(player.current?.uid).toBe(c.uid);
		expect(displayed()?.uid).toBe(c.uid); // seeded synchronously so the surface is not blank
	});

	it('Test C2 — the cold-start seed does NOT overwrite an existing displayed track (the defer)', async () => {
		(player.play as unknown as { mockRestore(): void }).mockRestore?.();
		mockEnsure.mockReset();
		const a = mk('netease', 'fa', 'A', 'Displayed');
		const stubB = stub('qq', 'fb', 'B', 'Loading');
		...
		player.displayed = a; // a song is already displayed
		void player.play(stubB);

		expect(player.current?.uid).toBe(stubB.uid); // current advanced
		expect(displayed()?.uid).toBe(a.uid); // displayed UNCHANGED — `??=` did not overwrite
	});

	it('Test D — restore() sets displayed to the restored track (reload resume shows it immediately)', async () => {
		const STATE_KEY = 'openmusic:player:v1';
		const cur = mk('netease', 'dr', 'Artist', 'Restored');
		...
		mockEnsure.mockResolvedValue(mk('netease', 'dr', 'Artist', 'Restored'));

		player.displayed = null;
		await player.restore();

		// restore() assigns current the restored track and sets displayed to it directly (no `playing`).
		expect(displayed()?.uid).toBe(cur.uid);
		expect(displayed()?.uid).toBe(player.current?.uid);
	});
});
