...
(A) **The shared song arrives armed, and behaves differently depending on whether anything is already playing.** Opening a share link (`/song/<artist>/<title>?play=<token>`) currently lands on a page where the first tap pays the full cold-resolve gap (~2.7s upstream detail + device-to-CDN, see `cold-start-budget-measured`). Two cases:
