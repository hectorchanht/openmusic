...
- [ ] 30-01-PLAN.md — Carrier-free songShareUrl/entityCardUrl + path-segment codec + ogImageUrl; drop share-time s2t; §F.20 share.test.ts rewrite (OG-PATH-02/OG-ZH-01) [wave 1]
- [ ] 30-02-PLAN.md — Extract `$lib/proxy/deezer-cover.ts` + rewire /api/deezer/search; deezer-endpoint.test.ts passes UNMODIFIED as the proof (OG-EP-03) [wave 1]
- [ ] 30-03-PLAN.md — `$lib/proxy/og-cover.ts` tier chain (Deezer→iTunes→kuwo, 2.5s deadline) + streaming `/api/og` route with two own-origin cache layers + endpoint tests (OG-EP-01/02/03) [wave 2, depends 30-02]
- [ ] 30-04-PLAN.md — New `/song/[artist]/[title]` + `/album/[artist]/[name]` routes + first loader tests + PageOg origin/og:type fix + APK-safe `<img>` (OG-PATH-01/OG-PAGE-01) [wave 2, depends 30-01]
