...
(A) **The shared song arrives armed, and behaves differently depending on whether anything is already playing.** Opening a share link (`/song/<artist>/<title>?ci=<coverToken>` — CORRECTED: the live link is carrier-free apart from the cover token; `?play=<token>` is a LEGACY home-route shape with no outbound caller left) currently lands on a page where the first tap pays the full cold-resolve gap (~2.7s upstream detail + device-to-CDN, see `cold-start-budget-measured`). Two cases:

  - **Nothing playing (cold arrival).** The shared song becomes the STARTER: it appears in the now-bar / Now Playing page already resolved, cover and metadata seeded, `audio.src` armed — paused, ONE tap from sound. This is exactly the shape `player.restore()` already produces on a PWA reopen (`player.svelte.ts:553` — sets `current`, seeds `resolvedCover` through the pinned -> track -> uid-cache -> name-cache chain, `syncMetadata()`, `ensureTrackDetails`, sets src, never calls `play()`). Do NOT autoplay: mobile autoplay policy will reject it, and the share-link navigation is not an in-page gesture.
  - **Something already playing (warm arrival).** The shared song is queued and played like a tap on a Related row — `NpRelated.svelte` `relatedTapPlay`: `playNext(track, { pin: false })` then `play(track, { fresh: false })`. It plays INSTANTLY (the audio element is already unlocked) and the QUEUE SHAPE SURVIVES: no `fresh: true`, so no history weave, no re-anchor of `upNextAnchorUid`, no `removedUids` clear, no tail regeneration — every row the listener had lined up stays, only the `.playing` highlight moves. `pin: false` so the shared song does not survive later queue resets.

  Both cases must respect the `playGen` / `pendingGen` guards, route network through the `apiFetch` governor, and add no eager blob pre-buffer (`api-fetch-flood-freeze`).
...
- [x] 39-02-PLAN.md — home-layout model: 17 section ids, 27-region allowlist, genre pool, versioned migration, reorderListed; i18n keys in 15 locales (wave 1)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 39-03-PLAN.md — Edge route /api/charts (Apple RSS · KKBOX · YouTube Charts) with serve-stale over caches.default (wave 2)
- [x] 39-04-PLAN.md — Deezer ?genre= branch + client chart services incl. client-side legacy iTunes genre feed (wave 2)
- [x] 39-05-PLAN.md — home-charts pure helpers: planChartShelves (hidden = 0 requests), samplePicks, Intl region labels, album href, cache keys (wave 2)
- [x] 39-06-PLAN.md — Settings fields: homeChartRegion / homeExtraRegions / homeChartGenres (no migration yet) (wave 2)

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 39-07-PLAN.md — Home page wiring: v3 pools+picks cache, chart task runner, gated classic fetches, seven shelves, album CompactRow (wave 3)
